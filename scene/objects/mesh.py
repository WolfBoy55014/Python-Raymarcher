...
class MeshObject(SceneObject):
    ...
    def getSDF(self, ray: Ray) -> float:
        ...
        self.nearest_face = np.argmin(distances)
    ...
    def _triangle(p, a, b, c) -> float:
        ...
        e = min(
            min(
                _dot2(
                    ba * clamp(np.dot(ba, pa) / _dot2(ba), 0.0, 1.0) - pa
                ),
                _dot2(
                    cb * clamp(np.dot(cb, pb) / _dot2(cb), 0.0, 1.0) - pb
                ),
            ),
            _dot2(
                ac * clamp(np.dot(ac, pc) / _dot2(ac), 0.0, 1.0) - pc
            ),
        )

        f = np.dot(nor, pa) * np.dot(nor, pa) / _dot2(nor)

        return math.sqrt(e if d < 2.0 else f) - 0.02

    def getMaterial(self):
        return self.material

    def getNormal(self, ray: Ray):
        return self.normals[self.nearest_face]
